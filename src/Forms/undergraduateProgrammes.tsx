            ...
            <div className="sm:flex sm:items-start">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 className="text-lg leading-6 font-medium text-gray-900 mt-2">
                  Undergraduate Form Application Submitted Successfully
                </h3>
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    Thank you for your interest in studying in Germany! We have received your Undergraduate Form Application and will review it shortly.
                  </p>
...
const EnrollmentForm: React.FC<EnrollmentFormProps> = ({ onSuccess }) => {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 2) {
      if (!formData.highSchoolSystem) newErrors.highSchoolSystem = 'Please select an option';
      if (!formData.diplomaStatus) newErrors.diplomaStatus = 'Please select an option';
    ...
    if (validateStep(currentStep)) {
      try {
        ...
        if (formData.highSchoolCertificate) {
          formDataToSend.append('highSchoolCertificate', formData.highSchoolCertificate);
        }
        if (formData.diplomaCertificate) {
          formDataToSend.append('diplomaCertificate', formData.diplomaCertificate);
        }
        if (formData.transcripts) {
          formDataToSend.append('transcripts', formData.transcripts);
        }
        
        // Send the form data
        await apiClient.post('/apply/undergraduate', formDataToSend, {
          headers: {
      ...
      <div className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Which high school education system did you attend? If you have not yet graduated from highschool, please select "N/A."
          ...
          <div className="mt-2 space-y-2">
            {[
              ...
              'Others',
              'N/A (not yet graduated from high school)'
            ].map((option) => (
              <div key={option} className="flex items-center">
                <input
                  type="radio"
                  id={`highSchool-${option}`}
                  name="highSchoolSystem"
                  value={option}
                  checked={formData.highSchoolSystem === option}
                  onChange={() => updateFormData('highSchoolSystem', option)}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                />
                <label htmlFor={`highSchool-${option}`} className="ml-2 block text-sm text-gray-700">
                  {option}
                </label>
              </div>
            ))}
          </div>
          {errors.highSchoolSystem && (
            <p className="mt-1 text-sm text-red-600">{errors.highSchoolSystem}</p>
          )}
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Have you studied/finished a diploma in your home country before?
            <span className="text-red-500">*</span>
          </label>
          <div className="mt-2 space-y-2">
            {[
              'Yes I have already completed an undergraduate degree',
              'Yes I have already completed a diploma',
              'Yes I have partially studied for 1 year but I have completed the first academic year (transcripts are available)',
              'I have not studied or done a diploma'
            ].map((option) => (
              <div key={option} className="flex items-center">
                <input
                  type="radio"
                  ...
                  name="diplomaStatus"
                  value={option}
                  checked={formData.diplomaStatus === option}
                  onChange={() => updateFormData('diplomaStatus', option)}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                />
                <label htmlFor={`diploma-${option}`} className="ml-2 block text-sm text-gray-700">
                  {option}
                </label>
              </div>
            ))}
          </div>
          {errors.diplomaStatus && (
            <p className="mt-1 text-sm text-red-600">{errors.diplomaStatus}</p>
          )}
        </div>
  ...
  const renderDocuments = () => (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="space-y-8">
        ...
        <FileUpload
          label="Kindly upload your diploma certificate."
          id="diplomaCertificate"
          file={formData.diplomaCertificate}
          onChange={(file) => updateFormData('diplomaCertificate', file)}
      ...
      <div className="pt-6 border-t border-gray-200">
        ...
        Please note that, depending on your qualifications, a consultation fee may be applicable to address your needs. This will be communicated to you after we review your application.
         For certain opportunities, no consultation fee is required.
        </p>
        ...
        <div className="mt-8 flex justify-between">
          ...
          {currentStep < totalSteps ? (
            <button
              type="button"
              onClick={nextStep}
              className="ml-auto px-6 py-2 bg-button-50 text-white rounded-md hover:bg-orange-700 transition-colors duration-200"
              disabled={isSubmitting}
            >
              Continue
            </button>
            ...
            <button
              type="submit"
              className="ml-auto px-6 py-2 bg-button-50 text-white rounded-md hover:bg-green-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Submitting...' : 'Submit Undergraduate Form Application'}
            </button>
          )}
        </div>
      </form>
...
// Main UndergraduateFormModal Component
function UndergraduateFormModal() {
    ...
    <div className="min-h-screen flex flex-col items-center py-8 px-4">
      <header className="w-full max-w-4xl flex items-center justify-center mb-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-blue-900">German Undergraduate Enrollment</h1>
        </div>
      </header>
      ...
      <main className="w-full max-w-4xl">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="p-6 bg-primary-700 text-white">
            <h2 className="text-xl font-semibold">Enrollment for Undergraduate Programmes in Germany</h2>
            <p className="mt-2 text-blue-100">
              Are you interested in enrolling in undergraduate programs in Germany? Register your profile with us. 
...
export default UndergraduateFormModal;