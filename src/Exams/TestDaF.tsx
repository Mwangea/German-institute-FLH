import React from 'react';
import { Calendar, FileText, CheckCircle2, ArrowLeft, ExternalLink } from 'lucide-react';
import { Link } from 'react-router-dom';

const TestDaFExam: React.FC = () => {
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section */}
      <section className="relative py-16 bg-gradient-to-b from-gray-900 to-gray-800">
        <div className="absolute inset-0">
          <img
            src="/TestDaF.jpg"
            alt="Students preparing for TestDaF"
            className="w-full h-full object-cover opacity-20"
          />
          <div className="absolute inset-0 bg-gradient-to-r from-gray-900/70 to-gray-900/30 mix-blend-multiply"></div>
        </div>
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center justify-center mb-6">
              <span className="px-3 py-1 bg-primary-500 text-white text-sm font-medium rounded-full">
                B2-C1 Level
              </span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-bold text-white text-center mb-6">
              TestDaF Examination
            </h1>
            <p className="text-xl text-gray-300 text-center mb-8">
              The German language test for university admission
            </p>
          </div>
        </div>
      </section>

      {/* Main Content */}
      <section className="py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-6">
            <Link 
              to="/programs/german-exams" 
              className="inline-flex items-center text-black hover:text-primary-200 transition-colors"
            >
              <ArrowLeft className="w-5 h-5 mr-2" />
              Back to German Exams
            </Link>
          </div> 
          <div className="max-w-3xl mx-auto">
            {/* Ideal For Section */}
            <div className="bg-white rounded-xl shadow-sm p-8 mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">
                Ideal if you'd like to...
              </h2>
              <ul className="space-y-4">
                {[
                  "Study, conduct research or work in Germany",
                  "Take a language exam recognized by all German universities",
                  "Provide evidence of your German proficiency to research institutes and employers"
                ].map((item, index) => (
                  <li key={index} className="flex items-start gap-3">
                    <CheckCircle2 className="w-6 h-6 text-primary-500 flex-shrink-0 mt-1" />
                    <span className="text-gray-700">{item}</span>
                  </li>
                ))}
              </ul>
            </div>

            {/* Description */}
            <div className="bg-white rounded-xl shadow-sm p-8 mb-8">
              <p className="text-gray-700 mb-6 leading-relaxed">
                The TestDaF is an advanced-level language exam covering levels B2 to C1 on the six-level scale of competence laid down in the Common European Framework of Reference for Languages (CEFR).
              </p>
              <p className="text-gray-700 mb-6 leading-relaxed">
                The certificate issued upon successful completion of all four sections at TestDaF level 4 provides admission to almost any subject at German universities. It also serves as internationally recognized evidence of language skills for academic professions.
              </p>
              <p className="text-gray-700 leading-relaxed">
                No specialist knowledge is required to take the exam.
              </p>
            </div>

            {/* Global Presence */}
            <div className="bg-white rounded-xl shadow-sm p-8 mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">
                Global Presence
              </h2>
              <p className="text-gray-700 mb-4 leading-relaxed">
                The TestDaF exam can only be taken at licensed test centers. There are currently around 450 test centers worldwide.
              </p>
              <a 
                href="https://www.testdaf.de/de/testzentren/der-testdaf-an-ihrem-testzentrum/die-testzentren-von-gast/" 
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center text-primary-600 hover:text-primary-700 hover:underline"
              >
                <ExternalLink className="w-4 h-4 mr-2" />
                Find TestDaF Test Centers
              </a>
            </div>

            {/* Exam Information */}
            <div className="grid md:grid-cols-2 gap-8 mb-8">
              <div className="bg-white rounded-xl shadow-sm p-8">
                <div className="flex items-center gap-3 mb-4">
                  <Calendar className="w-6 h-6 text-primary-500" />
                  <h3 className="text-xl font-semibold text-gray-900">Examination Dates</h3>
                </div>
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                  <p className="text-gray-700 mb-2">
                    TestDaF exam dates are set centrally by the TestDaF Institute.
                  </p>
                  <p className="text-gray-700">
                    Places are allocated on a first-come, first-served basis.
                  </p>
                </div>
              </div>

              <div className="bg-white rounded-xl shadow-sm p-8">
                <div className="flex items-center gap-3 mb-4">
                  <FileText className="w-6 h-6 text-primary-500" />
                  <h3 className="text-xl font-semibold text-gray-900">Important Information</h3>
                </div>
                <p className="text-gray-700 mb-4">
                  The exam consists of four sections: Reading, Listening, Writing, and Speaking.
                </p>
                <p className="text-gray-700">
                  Results are typically available 6-8 weeks after the exam date.
                </p>
              </div>
            </div>

            {/* CTA */}
            <div className="bg-gradient-to-br from-primary-600 to-primary-700 rounded-xl shadow-lg p-8 text-center relative overflow-hidden">
              <div className="absolute inset-0 bg-pattern opacity-10"></div>
              <div className="relative z-10">
                <h2 className="text-2xl font-bold text-white mb-4">
                  Ready to Take the TestDaF?
                </h2>
                <p className="text-white/90 mb-6">
                  Prepare for university studies in Germany with this internationally recognized language exam.
                </p>
                <div className="flex flex-wrap justify-center gap-4">
                  <button className="px-8 py-3 bg-white text-primary-600 rounded-lg hover:bg-gray-100 transition-all duration-300 transform hover:scale-105 shadow-lg">
                    Find a Test Center
                  </button>
                  <button className="px-8 py-3 bg-transparent text-white border border-white rounded-lg hover:bg-white hover:text-primary-600 transition-all duration-300 transform hover:scale-105">
                    Learn About Preparation
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default TestDaFExam;